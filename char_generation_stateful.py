...
model = Sequential()
model.add(LSTM(256, batch_input_shape=(batch_size, max_len, num_chars), stateful=True, shuffle=False))
...
for epoch in range(0, 2):
	...
	# model.fit(X_train, Y_train, batch_size=batch_size, nb_epoch=1, shuffle=False)
	model.reset_states()
	...
	for i in range(0, gen_chars):
		...
		pred = model.predict_on_batch(inp)[0]
